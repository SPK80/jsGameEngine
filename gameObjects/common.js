...
export class IGameObject extends IUpdating {
	...
	decorateBody(_class, ...params) { throw ('decorateBody must be implemented') }
}

export class IDrawingGameObject extends IGameObject {
	get drawing() { throw ('drawing must be implemented') }
	decorateDrawing(_class, ...params) { throw ('decorateDrawing must be implemented') }
}
